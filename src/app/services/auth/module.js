...
angular.module('service.auth', [])
    ...
    .service('authService', function ($q, $rootScope, $http, localStorageService, userService)
    {
        return {
            login: function (username, password) {
                var authService = this;
                return $http.post($rootScope._apiUrlBase + 'v1/login', {
                    username:username,
                    password:password
                }).then(function (data) {
            ...
            {
                var authService = this;
                return $http.post($rootScope._apiUrlBase + 'v1/forgot_password', {
                    username:username
                }).then(function () {
                    $rootScope._loginAttempts = 0;
                });
