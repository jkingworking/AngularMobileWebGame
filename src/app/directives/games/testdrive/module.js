...
angular.module('directive.games.testDrive', [])
    ...
    .controller('gameTestDriveController', function ($scope, $rootScope, $http, ModalService, $timeout)
    {
        ...
        function init()
        {
            ...
            var customers = ($scope.activity.condition_param) ? $scope.activity.condition_param.split(',') : null;

            $scope.customers = arrayUnique(customers);
        ...
        $scope.submitInfo = function ()
        {
            if ($scope.formData)
            {
                ...
                $http
                    .post($rootScope._apiUrlBase + 'v1/game/play/' + $scope.gameData.id, postData)
                    .then(
                    function (data)
                    {
