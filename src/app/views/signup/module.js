...
angular.module('view.signUp', ['service.auth'])
    ...
    .config(function ($stateProvider)
    {
        $stateProvider
            .state('signUp', {
                ...
                resolve: {
                    groupData: function ($http, $rootScope, $stateParams, $q)
                    {
                        $rootScope._loading = true;
                        return $http
                            .get($rootScope._apiUrlBase + 'v1/signup/groups')
                            .then(function (data)
                            {
    ...
    .controller('signUpController', function (authService, $rootScope, $scope, $state, localStorageService, $http, groupData, $stateParams)
    {
        ...
        $scope.signUpUser = function ()
        {
            $rootScope._loading = true;
            $scope.errors = false;
            $http.post($rootScope._apiUrlBase + 'v1/signup/user', $scope.signup).then(
                function (data)
                {
                    $rootScope._loading = false;
